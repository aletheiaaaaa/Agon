  ...
  void Adam<Ts...>::load_from_bin(const std::string& path_str) {
    ...
    std::apply([&](auto&... param_vecs) {
      (std::ranges::for_each(param_vecs, [&](auto& param_ref) {
        ...
        in.read(reinterpret_cast<char*>(vel.data()), param.numel() * sizeof(T));
      }), ...);
    }, this->parameters_.data);
  }
  ...
  void Adam<Ts...>::save_to_bin(const std::string& path_str) const {
    ...
    std::apply([&](auto&... param_vecs) {
      (std::ranges::for_each(param_vecs, [&](auto& param_ref) {
        ...
        out.write(reinterpret_cast<const char*>(vel.data()), param.numel() * sizeof(T));
      }), ...);
    }, this->parameters_.data);
  }

  template class Adam<std::tuple<Parameter<float>>>;
  template class Adam<std::tuple<Parameter<double>>>;
  template class Adam<std::tuple<Parameter<float>, Parameter<double>>>;
}