...
namespace agon::simd {
  ...
  inline Vec add(const Vec& a, const Vec& b) {
    if constexpr (std::is_same_v<Vec, VecI8<Arch::AVX2>>) {
      return Vec(_mm256_add_epi8(a.data, b.data));
    } else if constexpr (std::is_same_v<Vec, VecI16<Arch::AVX2>>) {
      return Vec(_mm256_add_epi16(a.data, b.data));
    } else if constexpr (std::is_same_v<Vec, VecF32<Arch::AVX2>>) {
      return Vec(_mm256_add_ps(a.data, b.data));
    } else if constexpr (std::is_same_v<Vec, VecF64<Arch::AVX2>>) {
      return Vec(_mm256_add_pd(a.data, b.data));
    }
  }
#elif defined(__SSE4_1__)
