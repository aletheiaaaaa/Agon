...
namespace agon::simd {
  ...
  inline Vec fmadd(const Vec& a, const Vec& b, const Vec& c) {
    if constexpr (std::is_same_v<Vec, VecF32<Arch::AVX512>>) {
      return Vec(_mm512_fmadd_ps(a.data, b.data, c.data));
    } else if constexpr (std::is_same_v<Vec, VecF64<Arch::AVX512>>) {
      return Vec(_mm512_fmadd_pd(a.data, b.data, c.data));
    }
  }
  ...
  inline Vec fmadd(const Vec& a, const Vec& b, const Vec& c) {
    if constexpr (std::is_same_v<Vec, VecF32<Arch::AVX2>>) {
      return Vec(_mm256_fmadd_ps(a.data, b.data, c.data));
    } else if constexpr (std::is_same_v<Vec, VecF64<Arch::AVX2>>) {
      return Vec(_mm256_fmadd_pd(a.data, b.data, c.data));
    }
  }
  ...
  inline Vec fmadd(const Vec& a, const Vec& b, const Vec& c) {
    if constexpr (std::is_same_v<Vec, VecF32<Arch::SSE4_1>>) {
      return Vec(_mm_add_ps(_mm_mul_ps(a.data, b.data), c.data));
    } else if constexpr (std::is_same_v<Vec, VecF64<Arch::SSE4_1>>) {
      return Vec(_mm_add_pd(_mm_mul_pd(a.data, b.data), c.data));
    }
  }#else
  template<typename Vec>
