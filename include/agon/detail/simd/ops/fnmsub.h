...
namespace agon::simd {
  ...
  inline Vec fnmsub(const Vec& a, const Vec& b, const Vec& c) {
    if constexpr (std::is_same_v<Vec, VecF32<Arch::AVX512>>) {
      return Vec(_mm512_fnmsub_ps(a.data, b.data, c.data));
    } else if constexpr (std::is_same_v<Vec, VecF64<Arch::AVX512>>) {
      return Vec(_mm512_fnmsub_pd(a.data, b.data, c.data));
    }
  }
  ...
  inline Vec fnmsub(const Vec& a, const Vec& b, const Vec& c) {
    if constexpr (std::is_same_v<Vec, VecF32<Arch::AVX2>>) {
      return Vec(_mm256_fnmsub_ps(a.data, b.data, c.data));
    } else if constexpr (std::is_same_v<Vec, VecF64<Arch::AVX2>>) {
      return Vec(_mm256_fnmsub_pd(a.data, b.data, c.data));
    }
  }
  ...
  inline Vec fnmsub(const Vec& a, const Vec& b, const Vec& c) {
    if constexpr (std::is_same_v<Vec, VecF32<Arch::SSE4_1>>) {
      return Vec(_mm_sub_ps(_mm_sub_ps(_mm_setzero_ps(), _mm_mul_ps(a.data, b.data)), c.data));
    } else if constexpr (std::is_same_v<Vec, VecF64<Arch::SSE4_1>>) {
      return Vec(_mm_sub_pd(_mm_sub_pd(_mm_setzero_pd(), _mm_mul_pd(a.data, b.data)), c.data));
    }
  }#else
  template<typename Vec>
